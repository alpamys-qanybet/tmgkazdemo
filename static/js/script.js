(function($) {
	...
	function enableCarousel(el) {
		jQuery(el).removeClass('carousel-hide');
        jQuery(el).siblings().addClass('carousel-hide');
	}
	...
	$(function() {
		...
		$('#carousel-example-generic').on('slide.bs.carousel', function (ev) {
			var item = ev.relatedTarget.id;
			item = item.replace('carousel-item-', '');
			...
			enableCarousel('#carousel-right-'+item);
		});
